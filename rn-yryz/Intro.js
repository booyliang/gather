...
import { StyleSheet, TextInput, Alert, NativeModules, PixelRatio } from 'react-native';
import {
	Content, Container, Right, Button, Text, Card, ListItem, View, Touchable, Left, Body, Title, Nav, List, Thumbnail, Form, Input, Textarea, Label, Picker, Item, Radio, ActionSheet, H3, Root, Image, withUser, withNavigation, CheckBox
} from '../../components';
...
export default class Intro extends React.Component {
	...
	// 获取用户信息
	async useSummary() {
		const userRes = await http.get(`/services/app/v1/user/single/local/${this.props.user.userId}`);
	...
	renderProvince(key, i) {
		return (
			<ListItem style={styles.List} key={i} onPress={this.onClick.bind(this, key)}>
				<Text value={key}>{key}</Text>
				{this.state.selectedProvince === key ? <Right>
					<CheckBox style={styles.Radio} checked={true} />
				</Right> : null}
			</ListItem>
		);
	...
	sheet() {
		ActionSheet.show({
			...
			if (BUTTONS[index] === '拍照') {
				...
				let cameraPermission = await NativeModules.YCommon.getCameraPermission()
				if (cameraPermission == '100') {
					...
					<Content>
						<List>
							...
							<ListItem style={styles.mainList} onPress={this._cityClick.bind(this)}>
								...
								<View style={styles.mainRight}>
									<Text style={styles.textColor}>{this.state.selectedProvince}  </Text>
									<Text style={styles.textColor}>{this.state.selectedCity ? this.state.selectedCity.substring(0, 5) : null}  </Text>
									<Text style={styles.textColor}>{this.state.selectedArea ? this.state.selectedArea.substring(0, 6) : null}</Text>

									{/* <Text>{this.props.city ? this.props.city[0] : null}  </Text>
									<Text>{this.props.city ? this.props.city[1].substring(0, 5) : null}  </Text>
									<Text>{this.props.city ? this.props.city[2].substring(0, 6) : null}  </Text> */}
									<YIcon name="arrow-right" style={styles.arrowRight} />
								</View>
							</ListItem>
							...
							<ListItem style={styles.mainEnd}>
								...
								<View style={styles.mainEndRight}>
									<Text note>
										<Text style={styles.maxText}>{this.state.maxText || this.state.userSign.length}</Text>
										/30
									</Text>
								</View>
							</ListItem>
